...
public class TournamentView extends ViewWithUiHandlers<TournamentUiHandlers>
		...
		final Column<MatchDto, SafeHtml> col2 = new Column<MatchDto, SafeHtml>(
			...
			public SafeHtml getValue(MatchDto object) {
				...
				if (object.getMatchPropertiesDto() != null) {
					...
					if (object.getState() == MatchParameters
							.getMATCHSTATE_QUEUE()) {
	...
	public void viewClasification(List<ClasificationDto> listClasificationDto,
		...
		panel.setType(PanelType.INFO);
		PanelHeader panelHeader = new PanelHeader();
		panelHeader.setText("Clasificación :: Jornada "
	...
	private void viewFinalMatch(final FinalMatchDto finalMatchDto,
			TournamentResources tournamentResources, PanelBody panelBody) {
		...
		paragraph4.setText(Double.toString(round(finalMatchDto.getMatchDto()
				.getMatchPropertiesDto().getPosessionHome() * 100, 2)));
		...
		paragraph7.setText(Integer.toString(finalMatchDto.getMatchDto()
				.getMatchPropertiesDto().getGoalsAway()));
		Paragraph paragraph8 = new Paragraph();
		paragraph8.setText(Double.toString(round(
				(1.0 - finalMatchDto.getMatchDto().getMatchPropertiesDto()
						.getPosessionHome()) * 100, 2)));
		...
		org.gwtbootstrap3.client.ui.Column column6 = new org.gwtbootstrap3.client.ui.Column(
				ColumnSize.XS_3, ColumnSize.SM_3, ColumnSize.MD_3,
				ColumnSize.LG_3);
		...
		download.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if (finalMatchDto.getMatchDto().getState() == MatchParameters
						.getMATCHSTATE_FINISH()) {
					Window.open(
							UploadParameters.getBASE_URL()
									+ "/serveMatchServlet?id="
									+ Long.toString(finalMatchDto.getMatchDto()
											.getId()), "_blank", "");
				} else {
					NotifySettings settings = NotifySettings.newSettings();
					settings.setType(NotifyType.INFO);
